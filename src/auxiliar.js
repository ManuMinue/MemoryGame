...
function start() {
	...
	gs.load("img/sprites.png",
		...
		function() {
			game = new MemoryGame(gs); // gs = Graphic Server
			game.initGame();
		});
}
...
var InputServer = function() {
	...
	var clientToCard = function(x,y) {
		...
		game.onClick(card); // Implementar onClick()
	};
	...
	var handleTouch = function(evt) { // Para pantallas táctiles y navegadores que admiten touch 
									  // (estos lanzan ambos, pero pone un pequeño delay a los de click)
        hasTouch = true;
